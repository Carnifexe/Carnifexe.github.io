...
window.startMainScript = async () => { 
    ...
    const scriptingDiv = document.getElementById("scriptingDiv");
    if (scriptingDiv) {
        document.body.innerHTML = scriptingDiv.innerHTML;
        savedBody = document.body.innerHTML;
    } else {
        console.error("scriptingDiv nicht gefunden!");
    }

    // Disclaimer öffnen, falls vorhanden
    if (typeof openDisclaimer === "function") {
        openDisclaimer();
    } else {
        console.error("openDisclaimer Funktion nicht gefunden!");
    }
    ...
    // CopyText-Funktion wieder aktivieren
